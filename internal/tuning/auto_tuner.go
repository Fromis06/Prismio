...
func (at *AutoTuner) checkRAMGuard() {
	...
	if err != nil {
		slog.Warn("AUTO-TUNER: Unable to read system memory; skipping this RAM guard cycle", "error", err)
		return
	}
	...
	case ramNormal:
		if v.UsedPercent >= ramCeilingPercent {
			...
			at.haltBatch(v.UsedPercent, "RAM ceiling exceeded for the first time; reducing batch size immediately")
		}
	...
	case ramThrottled:
		if v.UsedPercent < ramSafeResumePercent {
			...
			slog.Info("AUTO-TUNER: RAM usage is below the safe threshold; resuming FlushProbe adjustments",
			...
			at.haltBatch(v.UsedPercent, "RAM ceiling remains exceeded after one cycle; reducing batch size again")
		}
	}
}
